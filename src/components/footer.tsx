import Link from "next/link"
import { auth } from "@/auth"
import { getSiteConfig } from "@/lib/site-config"
import { LogoTile, Wordmark, BRAND_NAME } from "@/components/brand"

const SOCIAL_ICONS: Record<string, React.ReactNode> = {
  facebook: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
      <path d="M22 12c0-5.522-4.477-10-10-10S2 6.478 2 12c0 4.991 3.657 9.128 8.437 9.879V14.89h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.989C18.343 21.129 22 16.99 22 12z"/>
    </svg>
  ),
  linkedin: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
    </svg>
  ),
  instagram: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
    </svg>
  ),
  twitter: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
    </svg>
  ),
  telegram: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
      <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
    </svg>
  ),
  rednote: (
    <span className="inline-flex items-center justify-center w-5 h-5 rounded-[5px] border border-current text-[8px] font-black leading-none tracking-tight">
      RED
    </span>
  ),
}

const FALLBACK_SOCIALS = [
  { key: "facebook", name: "Facebook", href: "https://www.facebook.com/stacktalentx" },
  { key: "linkedin", name: "LinkedIn", href: "https://www.linkedin.com/company/stacktalentx" },
  { key: "instagram", name: "Instagram", href: "https://www.instagram.com/stacktalentx" },
]

export default async function Footer() {
  const config = await getSiteConfig()
  const session = await auth()
  const loggedIn = !!session?.user

  // Footer columns by audience: guests see both; a signed-in job seeker sees only
  // the Job Seekers column; a signed-in company sees only the Employers column.
  const role = session?.user?.role
  const showJobSeekers = !role || role === "INTERVIEWEE"
  const showEmployers = !role || role === "COMPANY"

  const socials = [
    config?.facebookUrl && { key: "facebook", name: "Facebook", href: config.facebookUrl },
    config?.instagramUrl && { key: "instagram", name: "Instagram", href: config.instagramUrl },
    config?.linkedinUrl && { key: "linkedin", name: "LinkedIn", href: config.linkedinUrl },
    config?.twitterUrl && { key: "twitter", name: "Twitter / X", href: config.twitterUrl },
    config?.rednoteUrl && { key: "rednote", name: "RedNote", href: config.rednoteUrl },
    config?.telegramUrl && { key: "telegram", name: "Telegram", href: config.telegramUrl },
  ].filter(Boolean) as { key: string; name: string; href: string }[]

  const displaySocials = socials.length > 0 ? socials : FALLBACK_SOCIALS

  // Build a wa.me link from the configured contact phone (digits only)
  const waNumber = config?.contactPhone?.replace(/[^\d]/g, "")
  const whatsappHref = waNumber ? `https://wa.me/${waNumber}` : null

  return (
    <footer className="border-t border-[#F0EEE8] bg-white mt-12">
      <div className="max-w-5xl mx-auto px-4 py-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 text-sm">

          {showJobSeekers && (
            <div className="space-y-3">
              <p className="font-bold text-[#1C1C1E]">Job Seekers</p>
              <ul className="space-y-2 text-[#6B7280]">
                <li><Link href="/" className="hover:text-[#F97316] transition-colors">Browse jobs</Link></li>
                <li><Link href="/dashboard" className="hover:text-[#F97316] transition-colors">My applications</Link></li>
                <li><Link href="/profile" className="hover:text-[#F97316] transition-colors">My profile</Link></li>
              </ul>
            </div>
          )}

          {showEmployers && (
            <div className="space-y-3">
              <p className="font-bold text-[#1C1C1E]">Employers</p>
              <ul className="space-y-2 text-[#6B7280]">
                {!loggedIn && (
                  <li><Link href="/register" className="hover:text-[#F97316] transition-colors">Register for free</Link></li>
                )}
                <li><Link href="/company/jobs/new" className="hover:text-[#F97316] transition-colors">Post a job</Link></li>
                <li><Link href="/company/jobs" className="hover:text-[#F97316] transition-colors">Manage jobs</Link></li>
              </ul>
            </div>
          )}

          <div className="space-y-3">
            <p className="font-bold text-[#1C1C1E]">Company</p>
            <ul className="space-y-2 text-[#6B7280]">
              <li><Link href="/about" className="hover:text-[#F97316] transition-colors">About us</Link></li>
              <li><Link href="/blog" className="hover:text-[#F97316] transition-colors">Blog</Link></li>
              <li><Link href="/pricing" className="hover:text-[#F97316] transition-colors">Pricing</Link></li>
              <li><Link href="/contact" className="hover:text-[#F97316] transition-colors">Contact us</Link></li>
              <li><Link href="/help" className="hover:text-[#F97316] transition-colors">Help Center</Link></li>
              <li><Link href="/terms" className="hover:text-[#F97316] transition-colors">Terms of Service</Link></li>
              <li><Link href="/privacy" className="hover:text-[#F97316] transition-colors">Privacy Policy</Link></li>
            </ul>
          </div>

          <div className="space-y-3">
            <p className="font-bold text-[#1C1C1E]">Follow Us</p>
            <div className="flex flex-wrap gap-3">
              {displaySocials.map((s) => (
                <a key={s.key} href={s.href} target="_blank" rel="noopener noreferrer"
                  aria-label={s.name}
                  className="text-[#9CA3AF] hover:text-[#F97316] transition-colors">
                  {SOCIAL_ICONS[s.key]}
                </a>
              ))}
            </div>

            {whatsappHref && (
              <a href={whatsappHref} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 mt-1 bg-[#25D366] hover:bg-[#1FB855] text-white text-xs font-bold px-3.5 py-2 rounded-full shadow-[0_4px_10px_rgba(37,211,102,0.3)] transition-colors">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M.057 24l1.687-6.163a11.867 11.867 0 0 1-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.82 11.82 0 0 1 8.413 3.488 11.82 11.82 0 0 1 3.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 0 1-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 0 0 1.512 5.26l-.999 3.648 3.736-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/>
                </svg>
                Chat on WhatsApp
              </a>
            )}
          </div>

        </div>

        <div className="mt-10 pt-6 border-t border-[#F0EEE8] flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            {config?.logoUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={config.logoUrl} alt={BRAND_NAME} className="h-7 w-auto object-contain" />
            ) : (
              <>
                <LogoTile size={26} variant="onDark" />
                <Wordmark className="text-sm font-extrabold text-[#1C1C1E] tracking-tight" />
              </>
            )}
          </div>
          <p className="text-xs text-[#B6B2A8]">
            © {new Date().getFullYear()} {BRAND_NAME}. All rights reserved. · Malaysia
          </p>
        </div>
      </div>
    </footer>
  )
}
