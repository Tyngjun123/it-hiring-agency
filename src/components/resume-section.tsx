...
export default function ResumeSection({ resumeUrl }: { resumeUrl: string | null | undefined }) {
  const [url, setUrl] = useState(resumeUrl ?? null)
            ...
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#EEEBE3]">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setViewOpen(false)}
                  className="w-7 h-7 rounded-full bg-[#F4F1EA] hover:bg-[#EEEBE3] text-[#3A3A3C] text-sm font-bold flex items-center justify-center transition-colors"
                >
                  ✕
                </button>
              </div>
